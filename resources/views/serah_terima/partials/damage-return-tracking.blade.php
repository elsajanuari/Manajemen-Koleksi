{{-- Partial: tracking pengembalian koleksi (pembatalan kerusakan) — dipakai di serah terima penyewaan --}}
@php
    $returnTrackingUrl = $isPengelola
        ? route('pengelola.penyewaan.handover.tracking-data', $penyewaan) . '?for=return'
        : route('penyewaan.requests.handover.tracking-data', $penyewaan) . '?for=return';
@endphp
    ...
    <div class="st-card-body">
        ...
        <div class="st-meta-grid" style="margin-top:0;">
            <div class="st-meta-cell">
                ...
                <div class="val">{{ $st->return_shipment_method ?? '-' }}</div>
            </div>
            <div class="st-meta-cell">
                <div class="lbl">No. Resi</div>
                <div class="val" style="font-family:monospace;">{{ $st->return_shipment_tracking ?? '-' }}</div>
            </div>
            <div class="st-meta-cell">
                <div class="lbl">Pengirim</div>
                <div class="val">{{ $st->return_shipment_officer ?? '-' }}</div>
            </div>
            <div class="st-meta-cell">
                <div class="lbl">Rencana Kirim</div>
                <div class="val">{{ $st->return_shipment_scheduled_at?->format('d M Y H:i') ?? '-' }}</div>
            </div>
            @if($st->return_shipping_cost !== null)
            <div class="st-meta-cell" style="background:linear-gradient(135deg,#fffbeb,#fef3c7);border-color:#fde68a;">
                ...
                <div class="val" style="color:#d97706;">Rp {{ number_format($st->return_shipping_cost, 0, ',', '.') }}</div>
            </div>
            @endif
        </div>
        @if($st->return_shipment_notes)
            <div class="st-catatan" style="margin-top:.875rem;">
                ...
                <div class="val">{{ $st->return_shipment_notes }}</div>
            </div>
        @endif
...
@else
    ...
    <div class="st-catatan" style="background:#f8fafc;border-color:#e2e8f0;">
        ...
        <div class="val">
            @if($isPengelola)
                Penyewa mengirim balik koleksi tanpa nomor resi. Monitor progres berdasarkan update dari penyewa.
            @else
    ...
    @if(!$isPengelola)
        ...
        @else
            <div class="st-card">
                ...
                <div class="st-card-body">
                    <form action="{{ route('penyewaan.requests.handover.return-status', $penyewaan) }}" method="POST">
                        @csrf
    ...
    @if(!empty($st->return_shipment_timeline))
        <div class="st-card">
            <div class="st-card-header">
                ...
                <h3>Timeline Pengembalian{{ $isPengelola ? ' (Update dari Penyewa)' : '' }}</h3>
            </div>
            <div class="st-card-body">
...
@if($isPengelola && !$st->collection_arrived_at)
    <div class="st-card" style="border-color:#6ee7b7;">
        ...
        <div class="st-card-body" style="background:linear-gradient(135deg,#f0fdf4,#ecfdf5);">
            <p style="font-size:.84rem;color:#065f46;margin:0 0 1.25rem;line-height:1.7;">
                Setelah koleksi benar-benar sudah diterima kembali di museum, konfirmasi untuk melanjutkan proses refund ke penyewa (termasuk ongkir pengembalian).
            </p>
