<x-app-layout>
                ...
                <div class="ps-hero-top">
                    <div>
                        <div class="ps-breadcrumb">
                            ...
                            <a href="{{ route('penyewaan.requests.show', ['penyewaan' => $penyewaan->id]) }}">SW-{{ str_pad($penyewaan->id, 5, '0', STR_PAD_LEFT) }}</a>
                            <span class="ps-breadcrumb-sep">/</span>
                    ...
                    <div class="ps-hero-actions">
                        <a href="{{ route('penyewaan.requests.show', ['penyewaan' => $penyewaan->id]) }}" class="ps-hero-btn ps-hero-btn-back">
        ...
        <div class="ps-content">
                ...
                <div class="ps-card-body">
                    <div class="ps-painting-row">
                        ...
                        <div class="ps-painting-info">
                            ...
                            <p>{{ $penyewaan->painting->artist }}</p>
                            <p style="margin-top:.2rem;font-family:monospace;font-size:.73rem;color:#94a3b8;">SW-{{ str_pad($penyewaan->id, 5, '0', STR_PAD_LEFT) }} &mdash; {{ $penyewaan->duration_days }} hari</p>
                        </div>
                    </div>
                    <div class="ps-cost-wrap">
                        <div class="ps-cost-row">
                            <span class="lbl">Subtotal Sewa ({{ $penyewaan->duration_days }} hari)</span>
                            <span class="val">Rp {{ number_format($subtotal, 0, ',', '.') }}</span>
                        </div>
                        <div class="ps-cost-row">
                            <span class="lbl">Deposit Keamanan (50%)</span>
                        ...
                        <div class="ps-cost-row">
                            ...
                            <span class="val">Rp {{ number_format($penyewaan->shipping_cost, 0, ',', '.') }}</span>
                        </div>
                        @else
                        <div class="ps-cost-row">
                        ...
                        <div class="ps-info-box">
                            <p><strong>ℹ️ Informasi:</strong> Deposit akan dikembalikan setelah koleksi diperiksa dan dinyatakan dalam kondisi baik.</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="ps-card">
                <div class="ps-cta">
                    ...
                    <p class="ps-pay-note">Berbagai metode tersedia: Transfer Bank, Virtual Account, GoPay, QRIS, dan lainnya.</p>
                </div>
            </div>
    ...
    <script>
    const PAY_BTN_ORIGINAL = `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" style="width:16px;height:16px;"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z"/></svg>Bayar Sekarang — Rp {{ number_format($total, 0, ',', '.') }}`;

    const PROCESS_URL = '{{ route("penyewaan.requests.payment.process", $penyewaan) }}';
    const SUCCESS_URL = '{{ route("penyewaan.requests.payment.success", $penyewaan) }}';
    const STATUS_URL  = '{{ route("penyewaan.requests.payment.status", $penyewaan) }}';
    const FAILED_URL  = '{{ route("penyewaan.requests.payment.failed", $penyewaan) }}';
    ...
    async function startPayment() {
        ...
        try {
            ...
            if (data.error === 'Pembayaran sedang diproses.') {
                btn.disabled  = false;
                btn.innerHTML = PAY_BTN_ORIGINAL;
                alert('Ada transaksi sebelumnya yang masih aktif. Silakan cek status pembayaran Anda.');
    ...
    function openSnap(token, btn) {
        snap.pay(token, {
            ...
            onPending: () => { window.location.href = STATUS_URL; },
